...
bool string_compare(const char* s1, const char* s2); // Renamed strcmp to string_compare
...
void cmd_clear();
void cmd_hello();
void gdt_set_gate(int num, uint32_t base, uint32_t limit, uint8_t access, uint8_t gran); // Declaration added
...
public:
    ...
    // Add more overloads for other types as needed (e.g., unsigned int, etc.)
};

// Emulation of cin (basic string input)
class TerminalInput {
private:
...
public:
    ...
    // You might need more sophisticated methods to handle different input types
    // or to parse the input string. For simplicity, this only reads a string.
};

// Global instances of cout and cin
TerminalOutput cout;
TerminalInput cin;
...
extern "C" void keyboard_handler() { // Added extern "C" here
...
void process_command() {
    ...
    if (string_compare(command_buffer, "help")) { // Updated to string_compare
        cmd_help();
    } else if (string_compare(command_buffer, "clear")) { // Updated to string_compare
        cmd_clear();
    } else if (string_compare(command_buffer, "hello")) { // Updated to string_compare
        cmd_hello();
    } else {
