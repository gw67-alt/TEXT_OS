...
int fat32_format_volume(uint64_t ahci_base, int port, uint64_t total_sectors, const char* volume_label) {
    ...
    for (int i = 0; i < 11; i++) {
        if (i < label_len && volume_label[i] != '.') { dir_entry->name[i] = volume_label[i]; }
        else { dir_entry->name[i] = ' '; }
    }
...
int fat32_write_file(uint64_t ahci_base, int port, uint32_t dir_cluster, const char* filename, const void* buffer, uint32_t buffer_size) {
...
static int fat32_allocate_cluster_internal(uint64_t ahci_base, int port, uint32_t* new_cluster, uint8_t* sector_buffer) {
    if (!fat32_initialized) {
        cout << "ERROR: FAT32 not initialized in allocate_cluster.\n";
        return -1;
    }
    if (new_cluster == NULL) return -2;

    // Validate total_clusters to avoid infinite loops
    if (fat32_total_clusters == 0) {
        cout << "ERROR: Total clusters count is zero. Filesystem not properly initialized.\n";
        return -20;
    }

    // Read boot sector to get FSInfo location
    fat32_boot_sector_t* bs_temp = (fat32_boot_sector_t*)sector_buffer;
    int result = read_sectors(ahci_base, port, 0, 1, sector_buffer);
    if (result != 0) {
        cout << "ERROR: Failed read boot sector in allocate_cluster.\n";
        return -3;
    }

    uint16_t fs_info_sector = bs_temp->fs_info;
    if (fs_info_sector == 0 || fs_info_sector >= bs_temp->reserved_sectors) {
        cout << "ERROR: Invalid FSInfo sector location: " << fs_info_sector << "\n";
        return -4;
    }
    ...
    if (result != 0) {
        cout << "ERROR: Failed read FS Info sector " << fs_info_sector << ".\n";
        return -4;
    }

    // Check FSInfo signatures and get free count
    fat32_fsinfo_t* fs_info = (fat32_fsinfo_t*)sector_buffer;
    cout << "DEBUG: FSInfo Signatures: Lead=0x" << std::hex << fs_info->lead_signature
        << ", Struct=0x" << fs_info->structure_signature << std::dec << "\n";

    if (fs_info->lead_signature != 0x41615252 || fs_info->structure_signature != 0x61417272) {
        cout << "WARNING: Invalid FSInfo signature. Will use default values.\n";
        fs_info->free_cluster_count = 0xFFFFFFFF; // Unknown count
        fs_info->next_free_cluster = 2;           // Start from first data cluster
    }

    // Get free cluster count - but don't fail if it's 0
    uint32_t free_count = fs_info->free_cluster_count;
    if (free_count == 0) {
        cout << "WARNING: FSInfo indicates no free clusters, but will scan anyway.\n";
        // Continue anyway - don't return error here
    }
    else if (free_count != 0xFFFFFFFF) {
        cout << "INFO: FSInfo indicates " << free_count << " free clusters available.\n";
    }

    // Get starting cluster for scan
    uint32_t start_cluster = fs_info->next_free_cluster;
    if (start_cluster < 2 || start_cluster >= fat32_total_clusters + 2) {
        cout << "INFO: Invalid next_free_cluster " << start_cluster << ", resetting to 2.\n";
        start_cluster = 2;
    }

    // Initialize scan variables
    uint32_t current_scan_cluster = start_cluster;
    uint32_t checked_clusters = 0;
    uint32_t found_free_cluster = 0;

    cout << "DEBUG: Starting cluster scan at " << current_scan_cluster
        << ", Total clusters: " << fat32_total_clusters << "\n";

    // Scan for a free cluster
    while (checked_clusters < fat32_total_clusters) {
        ...
        if (result != 0) {
            cout << "ERROR: Failed read FAT sector " << fat_sector << " during scan.\n";
            return -6;
        }

        // Print first 10 FAT entries on first iteration for debugging
        if (checked_clusters == 0) {
            cout << "DEBUG: Examining first few FAT entries (sector " << fat_sector << "):\n";
            for (int i = 0; i < 10 && i < (FAT32_SECTOR_SIZE / 4); i++) {
                uint32_t fat_value = *((uint32_t*)(sector_buffer + (i * 4)));
                fat_value &= 0x0FFFFFFF;  // Mask the upper 4 bits
                cout << "  Cluster " << i << ": 0x" << std::hex << fat_value;
                //(fat_value == 0 ? " (FREE)" : " (USED)") << "\n";
            }
        }

        // Check if current cluster is free
        uint32_t cluster_value = *((uint32_t*)(sector_buffer + entry_offset_in_sector));
        cluster_value &= 0x0FFFFFFF;  // Mask reserved bits

        if (cluster_value == 0) {  // FAT32_FREE_CLUSTER should be 0
            found_free_cluster = current_scan_cluster;
            break;
        }

        // Progress to next cluster
        current_scan_cluster++;
        if (current_scan_cluster >= fat32_total_clusters + 2) {
            current_scan_cluster = 2;  // Wrap around to first data cluster
        }

        checked_clusters++;

        // Log progress periodically
        if (checked_clusters % 1000 == 0) {
            cout << "INFO: Checked " << checked_clusters << " clusters, currently at "
                << current_scan_cluster << "\n";
        }

        // Avoid infinite loop if we've wrapped around
        if (current_scan_cluster == start_cluster && checked_clusters > 0) {
            break;
        }
    }

    // Error if no free cluster found
    if (found_free_cluster == 0) {
        cout << "ERROR: No free clusters found after checking " << checked_clusters << " clusters.\n";
        cout << "       Total clusters: " << fat32_total_clusters << "\n";
        return -7;
    }

    // Mark the cluster as End-of-Chain
    cout << "INFO: Found free cluster " << found_free_cluster << ", marking as EOC.\n";
    result = fat32_update_fat_entry(ahci_base, port, found_free_cluster, FAT32_EOC, sector_buffer);
    if (result != 0) {
        cout << "ERROR: Failed to mark allocated cluster " << found_free_cluster << " as EOC.\n";
        return -8;
    }
    ...
    result = read_sectors(ahci_base, port, fs_info_sector, 1, sector_buffer);
    if (result != 0) {
        cout << "WARNING: Failed re-read FS Info for update.\n";
    }
    else {
        fs_info = (fat32_fsinfo_t*)sector_buffer;
        if (fs_info->lead_signature == 0x41615252 && fs_info->structure_signature == 0x61417272) {
            if (fs_info->free_cluster_count != 0xFFFFFFFF && fs_info->free_cluster_count > 0) {
                fs_info->free_cluster_count--;
            }
            fs_info->next_free_cluster = found_free_cluster + 1;
            if (fs_info->next_free_cluster >= fat32_total_clusters + 2) {
                fs_info->next_free_cluster = 2;
            }
            result = write_sectors(ahci_base, port, fs_info_sector, 1, sector_buffer);
            if (result != 0) {
                cout << "WARNING: Failed write updated FS Info.\n";
            }
        }
    }
    ...
    if (result != 0) {
        cout << "WARNING: Failed zero allocated cluster " << found_free_cluster << ".\n";
    }

    *new_cluster = found_free_cluster;
    cout << "INFO: Successfully allocated cluster " << found_free_cluster << "\n";
    return 0;
}
