...
TerminalIO& hex(TerminalIO& stream) {
    use_hex = true;  // Access the global variable directly
    return stream;
}

TerminalIO& dec(TerminalIO& stream) {
    use_hex = false;  // Access the global variable directly
    return stream;
}
#include "stdlib_hooks.h"
...
// Define the global formatting state variable (declared as extern in the header)
bool use_hex = false;
...
// This is a basic implementation that only handles a few format specifiers
// For a complete implementation, you'd need to handle all standard specifiers

// Forward declaration for terminal putchar
...
// Terminal output functions - these should call your existing terminal_putchar
...
// Keyboard input functions hooked into the existing keyboard handling code
TerminalIO& TerminalIO::operator>>(char* str) {
    // Use the external command_buffer and command_ready flag defined in the header
    ...
    *this << "> ";
    ...
    // Wait for the keyboard handler to set command_ready flag
    ...
    // Copy the command buffer to the provided string
    strcpy(str, command_buffer);
